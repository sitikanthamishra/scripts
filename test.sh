...
echo "<tr><td></td>"
for value in NODE  Disk_Usage_Per_Day_In_GB init_size_In_GB max_size_In_GB;do
...
for i in  `cat /root/nodes | awk '{print $1}'`; do
  ssh -t -q -o StrictHostKeyChecking=no -o  ConnectTimeout=10 -o ConnectionAttempts=1 root@$i '''
        NODE=$(hostname)
        MS=$(cat /tmp/sto-$(date  --date="today" +"%F")|awk  "{print \$4}"|tr -d G;)
        TU=$(cat /tmp/sto-$(date  --date="today" +"%F")|awk  "{print \$3}"|tr -d G;)
	YU=$(cat /tmp/sto-$(date  --date="yesterday" +"%F")|awk  "{print \$3}"|tr -d G;)
        CH=$(echo $TU - $YU | bc)
